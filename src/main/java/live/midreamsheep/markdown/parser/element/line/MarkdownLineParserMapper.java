...
import live.midreamsheep.markdown.parser.element.line.code.CodeParser;
import live.midreamsheep.markdown.parser.element.line.head.HeadParser;
    ...
    static{
        ...
        register("-", (lines,index,elements) -> {
            if(MarkdownParserStringUntil.isAlways(lines[index].trim().toCharArray(), '-')&&lines[index].trim().length()>=3){
                elements.addNewLine(new HorizontalLine());
                return index;
            }
            return -1;
        });
